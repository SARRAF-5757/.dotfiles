...
#
# Optional parameters:
# @raycast.icon https://www.pngplay.com/wp-content/uploads/6/Full-Battery-Symbol-Transparent-PNG.png
#
# Documentation:
# @raycast.description battery info
# @raycast.author Fahim Faisal
# @raycast.authorURL https://github.com/i3p9

#BATT_PERCENTAGE=$(pmset -g batt | awk '{ for( i=1; i<=NF; i++) if ($i ~ /%/) { sub(/%.*$/,"",$i); print $i; exit }}')
...
if [[ "$CHARGE_STATUS" == "charging;" ]]; then
    ...
    if [[ "$TIME_REMAINING" == "(no" ]]; then
        ...
        TO_SHOW="⚡${BATT}%  -  ${TIME_REMAINING_FORMATTED} to Full  -  ${CHARGE_WATT}  -  ${CYCLE_COUNT} Cycles"
        echo $TO_SHOW
    fi
    ...
    if [[ "$TIME_REMAINING" == "(no" ]]; then
        TO_SHOW="⚡${BATT}% | No Estimate Yet | ${CHARGE_WATT} | ${CYCLE_COUNT} Cycles"
        echo $TO_SHOW
    elif [[ "$FULLY_CHARGED_FLAG" = "TRUE" ]]; then
        TO_SHOW="⚡${BATT}% | Fully Charged | ${CHARGE_WATT} | ${CYCLE_COUNT} Cycles"
        echo $TO_SHOW
    else
        TO_SHOW="⚡${BATT}% | ${TIME_REMAINING_FORMATTED} to Full | ${CHARGE_WATT} - ${CYCLE_COUNT} Cycles"
        echo $TO_SHOW
